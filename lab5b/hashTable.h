...
public:
    // Constructor and public method declarations
    HashTable();
...
template <typename T>
HashTable<T>::HashTable() : size(1000) {
    slots.resize(size);
    occupied.resize(size, false);
}

template <typename T>
int HashTable<T>::hash(const T& item) const {
    return item % size;
}

template <typename T>
int HashTable<T>::rehash(int index) const {
    return (index + 1) % size;
}
...
template <typename T>
bool HashTable<T>::get(const T& item) const {
    int index = hash(item);
