...
double Fraction::getVal() const {
    return static_cast<double>(numerator) / denominator; //I read up on static_cast and I guess it's a "safer" way to cast variables
}
// I implemnted the fraction reduction in each operator overload. In hindsight, I should have made a separate function for it, or at least I think that would have been the better way.
// Guess I could always time it and see if there's a difference haha.
...
// I implemented the equality and inequality operators by multiplying the numerators and denominators of the two fractions and comparing them.
...
//This is the gcd function that was from the powerpoint. I don't think I changed anything in it.
...
int main() {
  ...
  // Equality
  cout << "(" << f1 << ") == (" << f1 << ") is " << (f1 == f1 ? "true" : "false") << endl;
  cout << "(" << f1 << ") == (" << f2 << ") is " << (f1 == f2 ? "true" : "false") << endl;
  cout << "(" << f1 << ") != (" << f2 << ") is " << (f1 != f2 ? "true" : "false") << endl;
  cout << "(" << f1 << ") != (" << f1 << ") is " << (f1 != f1 ? "true" : "false") << endl;
  cout << endl;
  ...
  cout << "(" << f1 << ") > (" << f2 << ") is " << (f1 > f2 ? "true" : "false") << endl;
  cout << "(" << f1 << ") > (" << f3 << ") is " << (f1 > f3 ? "true" : "false") << endl;
  cout << "(" << f2 << ") < (" << f1 << ") is " << (f2 < f1 ? "true" : "false") << endl;
  cout << "(" << f3 << ") < (" << f1 << ") is " << (f3 < f1 ? "true" : "false") << endl;
  cout << endl;
